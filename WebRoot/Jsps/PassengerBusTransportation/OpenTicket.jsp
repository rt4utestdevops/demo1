...
input[type="checkbox"]:checked+label span {
  ...
  <div class="row panel">
      <div class="divHeading">Open Trip</div>
  </div>
    <div class="row">
				...
				$.ajax({
	    			...
	    			totalPassenger=response.count;    				    				
	    				if(isRoundTrip==1){
	    					...
	    					$('#service-'+response.count).after('<div class="col-md-3"><button type="button" class="btn cancleSearch" id="btnCancelTrip" onclick="cancelTrip()">Open Trip</button></div>');
	    				}else{
	    				$('#onword-cancel').html(response.onwordStructure);
	    				$('#service-'+response.count).after('<div class="col-md-3"><button type="button" class="btn cancleSearch" id="btnCancelTrip" onclick="cancelTrip()">Open Trip</button></div>');
	    				}    				
...
function cancelTrip(){
	...
	$("#onword-cancel input:checked").each(function () {
    	...
    	$.ajax({
		url: "<%=request.getContextPath()%>/OpenTicketAction.do?param=getOpenTripInfoRound",
		data:{		
		tripID:tripID,
	    ...
	    success: function(response,data) {
	    	...
	    	if(isRoundTrip){
            window.location="<%=request.getContextPath()%>/Jsps/PassengerBusTransportation/CancellationInfo.jsp?name="+name+"&tripId="+tripID+"&openTicket="+'Open'
            }else{
	    	window.location="<%=request.getContextPath()%>/Jsps/PassengerBusTransportation/CancellationInfo.jsp?name="+name+"&tripId="+tripID+"&openTicket="+'OpenTicket'
	    	}
	    	}
	    }
	...
	$("#onword-cancel input:checked").each(function () {
      ...
      $("#btnCancelTrip").attr("disabled", "disabled"); 
    	...
    	onwordDate=$("#DateOfJourney").val();
    	$.ajax({
		url: "<%=request.getContextPath()%>/OpenTicketAction.do?param=getOpenTripInfoSingle",
		data:{		
		tripID:tripID,
		onwordSeat:onwordseats,
		emailId:email,
