...
var statutorycountStore=new Ext.data.JsonStore({
				url: '<%=request.getContextPath()%>/CarRentalAction.do?param=getStateWiseStatutoryCount',
        ...
        callback: function () {
            ...
            var store = new Ext.data.GroupingStore({
                ...
                proxy: new Ext.data.HttpProxy({
                    url: '<%=request.getContextPath()%>/CarRentalAction.do?param=getStatutoryDashboardDetails',
                    method: 'POST'
                }),
                remoteSort: false,
                sortInfo: {
	       			...
	       			listeners: 
	       			{
	        			click:
	        			{
	       					fn:function()
	       					{
								...
								Ext.Ajax.request({
								 				url: '<%=request.getContextPath()%>/CarRentalAction.do?param=saveCVSremarks',
												method: 'POST',
												params: 
												{
                    ...
                    },
                    {
                        ...
                        listeners: {
                            click: {
                                ...
                                ///else
                                //{
                                   window.location ="<%=request.getContextPath()%>/Jsps/CarRental/DashBoard.jsp?cutomerIDPassed="+<%=clientID%>;
                               // }
                                }
                            }
                        }
