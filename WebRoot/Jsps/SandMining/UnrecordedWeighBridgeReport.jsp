...
<html>
 <head>
 		<title>Unrecorded Weigh Bridge Report</title>		
</head>	    
	 <style>
  .x-panel-tl
	{
...
<script>
	...
	var outerPanel;
	var AssetNo;
 	var jspName='Unrecorded_Weigh_Bridge_Report';
   	var exportDataType = "int,string,string,string,string,string,date,date,number,number,string";
    var dtprev = dateprev;
	var dtcur = datecur;
	var dtnxt = datenext;
	var reportTypeId;
...
//************************ Combo for Customer Starts Here***************************************//
	...
	var reader = new Ext.data.JsonReader({
        idProperty: 'readerId',
        root: 'UnrecordedWeighBridgeRoot',
        totalProperty: 'total',
        fields: [{
            name: 'slnoIndex'
        },{
    ...
    var SummaryReportStore = new Ext.data.GroupingStore({
        ...
        proxy: new Ext.data.HttpProxy({
            url: '<%=request.getContextPath()%>/ILMS_MDP_GenetationAction.do?param=getUnrecordedWeighBridgeReport',
            method: 'POST'
        }),
        remoteSort: false,
    ...
    var filters = new Ext.ux.grid.GridFilters({
        ...
        filters: [{
            ...
            type: 'string',
            dataIndex: 'mdpNoDataIndex'
        }, {
            type: 'string',
            dataIndex: 'loadingLocationDataIndex'
        }, {
            type: 'string',
            ...
            type: 'numeric',
            dataIndex: 'unladenWeightDataIndex'
        }, {
            type: 'string',
    ...
    var createColModel = function (finish, start) {
        ...
        var columns = [
            new Ext.grid.RowNumberer({
                ...
                header: "<span style=font-weight:bold;>MDP No</span>",
                dataIndex: 'mdpNoDataIndex',
                filter: {
                    type: 'string'
                }
            }, {
                header: "<span style=font-weight:bold;>Loading Location</span>",
                dataIndex: 'loadingLocationDataIndex',
                filter: {
                    type: 'string'
                }
                ...
                header: "<span style=font-weight:bold;>Unladen Weight </span>",
                dataIndex: 'unladenWeightDataIndex',
                filter: {
                    type: 'numeric'
                }
    ...
    summaryGrid = getGrid('', '<%=NoRecordsFound%>', SummaryReportStore, screen.width - 35, 400, 22, filters, '<%=ClearFilterData%>', false, '', 10, false, '', false, '', true, '<%=Excel%>', jspName, exportDataType, true, '<%=PDF%>');
      ...
      Ext.onReady(function () {
        ...
        outerPanel = new Ext.Panel({
        	title:'Unrecorded Weigh Bridge Report',
            renderTo: 'content',
            standardSubmit: true,
            frame: true,
            cls: 'outerpanel',
