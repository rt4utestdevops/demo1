...
<html>
  <head>
    ...
    <title>My JSP 'CustomerWiseReport.jsp' starting page</title>
 ...
 <body>
  <%if (loginInfo.getStyleSheetOverride().equals("Y")){%>                  
   ...
   <jsp:include page="../Common/ImportJS.jsp" /><%} %>
   	...
   	<script>
   	var jspName = "CustomerWiseReport";
  	var exportDataType = "int,string,string,string,string,string,string,string,string,date,string,string,string,string,string";
    var outerPanel;
    var ctsb;
    var summaryGrid;
    var custId;
 ...
 var CvsCustomerstore = new Ext.data.JsonStore({
    url: '<%=request.getContextPath()%>/cashDispenseAction.do?param=getAllCustomer',
  ...
  var CvsCustomercombo = new Ext.form.ComboBox({
  		                ...
  		                click: {
  		                    fn: function () {
								 ...
								 if (Ext.getCmp('cvsCustomercomboId').getValue() == "") {
  		                            Ext.example.msg("Select CVS Customer");
                             ...
                             return;
                                ...
                                if (checkMonthValidation(startdates, enddates)) {
  		                        ...
  		                        //alert("ID "+Ext.getCmp('cvsCustomercomboId').getValue());
  		                        customerwiseReportStore.load({
                                   params: {
                                       CustId: Ext.getCmp('custcomboId').getValue(),
  		                               startdate: Ext.getCmp('StartDtId').getValue(),
  		                                enddate: Ext.getCmp('EndDtId').getValue(),
  		                                customerName:Ext.getCmp('cvsCustomercomboId').getValue(),
  		                                customerFullName:Ext.getCmp('cvsCustomercomboId').getRawValue(),
  		                                custName:Ext.getCmp('custcomboId').getRawValue(),
 		                                jspName:jspName
                                    }
                                });  
    ...
    var customerwiseReportStore = new Ext.data.GroupingStore({
        autoLoad: false,
        proxy: new Ext.data.HttpProxy({
            url: '<%=request.getContextPath()%>/CashVanReportAction.do?param=getcustomerwiseReport',
            method: 'POST'
        }),
        remoteSort: false,
        storeId: 'customerwiseReportId',
        reader: reader
    });
   ...
   summaryGrid = getGrid('', 'No Records Found', customerwiseReportStore, screen.width - 35, 400, 22, filters, 'Clear Filter Data', false, '', 10, false, '', false, '', true, 'Excel', jspName, exportDataType, true, 'PDF'); 
