...
margin-left:0px;
	...
	<div id="allPanels" class="main3" >
		...
		function plotSingleVehicle(regNo, latitude, longitude, location, dateTime,speed,category) {
		    ...
		    var content = '<div id="myInfoDiv" seamless="seamless" scrolling="no" style="overflow:hidden; width:100%; height: 100%; float: left; color: #000; background:#ffffff; line-height:100%; font-size:100%; font-family: sans-serif;">' + '<table>' + '<tr><td><b>Vehicle No:</b></td><td>' + regNo + '</td></tr>' + '<tr><td><b>Location:</b></td><td>' + location + '</td></tr>' + '<tr><td><b>Date Time:</b></td><td>' + dateTime + '</td></tr>' + '</table>' + '</div>';
	...
	function plotSingleVehicleForTrips(regNo, latitude, longitude, location, dateTime,speed,category) {
		    ...
		    var content = '<div id="myInfoDiv" seamless="seamless" scrolling="no" style="overflow:hidden; width:100%; height: 100%; float: left; color: #000; background:#ffffff; line-height:100%; font-size:100%; font-family: sans-serif;">' + '<table>' + '<tr><td><b>Location:</b></td><td>' + location + '</td></tr>' + '</table>' + '</div>';
		    infowindow = new google.maps.InfoWindow({
		        content: content,
		        maxWidth: 300,
		        marker: marker,
    ...
    fields: ['dateTime', 'regNo', 'location', 'speed', 'latitude', 'longitude','category','statusForTripPoints']
	    ...
	    if (Ext.getCmp('custcomboId').getValue() == "") {
		        setMsgBoxStatus('<%=SelectCustomerName%>');
		        return;
		    }
     ...
     var secondPanel = new Ext.Panel({
		...
		initialize();
		    if (Ext.getCmp('custcomboId').getValue() == "") {
		        ctsb.setStatus({
		            text: getMessageForStatus("<%=SelectCustomerName%>"),
		            iconCls: '',
		            clear: true
		        });
		    ...
		    if (grid.getSelectionModel().getCount() == 0) {
		        ctsb.setStatus({
		            text: getMessageForStatus("<%=NoRowSelected%>"),
		            iconCls: '',
		            clear: true
		        });
		        return;
		    ...
		    if (grid.getSelectionModel().getCount() > 1) {
		        ctsb.setStatus({
		            text: getMessageForStatus("<%=SelectSingleRow%>"),
		            iconCls: '',
		            clear: true
		        });
		        return;
