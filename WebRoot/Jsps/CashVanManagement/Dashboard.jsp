...
<jsp:include page="../Common/DashboardUtil.jsp" />
...
var VEHICLE_OFF_ROAD = new Ext.Panel({
		...
		items:[	{
				...
				items:[
				{
				html: '<div class="vehicleOffRoadDeatilsNavimage" id="vehicleOffdetailsnav" onclick="getVehicleOffRoadDetails(1);"></div>',
				xtype:'panel',
				//height:115,	
				bodyCssClass:'htmltotaldetailsnav',
...
//No Action to be taken on Dashboard
}
else{
	if(cutomerID !=""){
...
CheckSession();
 statutoryStore.load({
                        ...
                        callback  : function() {
    								...
    								['<%=driverLicense%>',  DriverLicenseDOE,   DriverLicenseExp]
          							...
          							//width:800,
          							//height:270,
          							vAxis:{title:'<%=vehicleCount%>',viewWindow: {
                        min:0
                    },maxValue:5,gridlines:{count:6},titleTextStyle: { italic: false,fontSize:13} }
        							};
 ...
 var statutorybargraph= new google.visualization.ColumnChart(document.getElementById('barchartddiv'));
    ...
    var cutomerID='<%=customerId%>';
      ...
      case 11:getDashBoardDetailsStatutoryAlert(cutomerID,11,'Statutory Details Goods and Token Tax Due for Expiry');break;
      case 12:getDashBoardDetailsStatutoryAlert(cutomerID,33,'Statutory Details Goods and Token Tax Expired');break;
...
if(<%=systemid%> == 257 && <%=customerId%> == 4522 && <%=userid%> == 198){
// For Secure Value History Analysis 
//No Action to be taken on Dashborad
}
else{
	if(cutomerID !="")
	{
