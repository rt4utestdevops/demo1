...
<script>
	...
	var denom500;
	var denom100;
	var denominationDetailsStore = new Ext.data.JsonStore({
	    url: '<%=request.getContextPath()%>/cashDispenseAction.do?param=getDenominationDetails',
	    root: 'denominationStoreRoot',
	    autoLoad: false,
	    remoteSort: true,
	    fields: ['denom5000', 'denom2000','denom1000','denom500','denom100']
    });
	...
	function  onCellClick (AssetGrid, rowIndex, columnIndex, e){
    	...
    	if('<%=reconcileHeadAuthority%>' == 1 && r.data['DenominationDataIndex'] != '2000'){
	...
	AssetGrid.on({
		...
		denominationDetailsStore.load({
			params:{cvsbusinessId:'<%=atmId%>',uniqueId:'<%=UniqueId%>',businessId:'<%=businessId%>'},
			callback: function(){
			var rec = denominationDetailsStore.getAt(0);
				denom5000 = rec.data['denom5000'];
				denom2000 = rec.data['denom2000'];
				denom1000 = rec.data['denom1000'];
				denom500 = rec.data['denom500'];
				denom100 = rec.data['denom100'];
			}
		});
...
/*	function shortExcessCalculation(e,denomination){
		var denomTotal = e.record.data['gernalDI'] + e.record.data['goodCashPysicalDI'] + e.record.data['badPhysicalCashDI'] + e.record.data['goodRejectedCashDI'] + e.record.data['badRejectedDI'];	
		if(denomination == '5000'){
		if(denomTotal > denom5000){
			e.record.set('shortDI',0);
			e.record.set('excessDI',denomTotal - denom5000);
		}else if(denomTotal < denom5000){
			e.record.set('shortDI',denom5000 - denomTotal);
			e.record.set('excessDI',0);
		}else{
			e.record.set('shortDI',0);
			e.record.set('excessDI',0);
		...
		if(denomTotal > denom2000){
			e.record.set('shortDI',0);
			e.record.set('excessDI',denomTotal - denom2000);
		}else if(denomTotal < denom2000){
			e.record.set('shortDI',denom2000 - denomTotal);
			e.record.set('excessDI',0);
		}else{
			e.record.set('shortDI',0);
			e.record.set('excessDI',0);
		...
		if(denomTotal > denom1000){
			e.record.set('shortDI',0);
			e.record.set('excessDI',denomTotal - denom1000);
		}else if(denomTotal < denom1000){
			e.record.set('shortDI',denom1000 - denomTotal);
			e.record.set('excessDI',0);
		}else{
			e.record.set('shortDI',0);
			e.record.set('excessDI',0);
		...
		if(denomTotal > denom500){
			e.record.set('shortDI',0);
			e.record.set('excessDI',denomTotal - denom500);
		}else if(denomTotal < denom500){
			e.record.set('shortDI',denom500 - denomTotal);
			e.record.set('excessDI',0);
		}else{
			e.record.set('shortDI',0);
			e.record.set('excessDI',0);
		...
		if(denomTotal > denom100){
			e.record.set('shortDI',0);
			e.record.set('excessDI',denomTotal - denom100);
		}else if(denomTotal < denom100){
			e.record.set('shortDI',denom100 - denomTotal);
			e.record.set('excessDI',0);
		}else{
			e.record.set('shortDI',0);
			e.record.set('excessDI',0);
    ...
    dataStoreForGrid.load({params:{cvsbusinessId:'<%=atmId%>',uniqueId:'<%=UniqueId%>',businessId:'<%=businessId%>',btnValue:'replinshMent'}});    
