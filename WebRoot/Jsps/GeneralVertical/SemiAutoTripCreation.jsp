		...
		<div class="modal-content">
			...
			<div class="row">
				 ...
				 <div class="form-group row">
                   ...
                   <div class="col-md-6">
                        <div id="dvMap" style="width: 730px; height: 417px; margin-top: 8px; border: 1px solid gray;"></div>
                        <div>
...
function saveData(){
 ...
 function openCheckPointModal(){
	...
	checkPointArray = [];
     ...
     function createCombo(counterI, hubId) {
        ...
        $(".after-add-more").before(html);
          ...
          $('#checkpoint' + counterI).select2({ width: '340px'});
          if (hubId > 0) {
	...
	if (index > -1) {
        ...
        for (var n = 0,leng = srcDestMarkers.length ; n < leng ; n++){
	        var currMar1 = srcDestMarkers[n];
		        if (removeId == currMar1.options.id){
		        	map.removeLayer(currMar1);
		        }
        }  
        $('#checkAddBtn').show();
      ...
      initialize("dvMap",new L.LatLng(<%=latitudeLongitude%>),'<%=mapName%>', '<%=appKey%>', '<%=appCode%>');

  function drawCheckpoints(id) {
    	
        myLatLngC = new L.LatLng($('#checkpoint' + id + ' option:selected').attr("latitude"), $('#checkpoint' + id + ' option:selected').attr("longitude"));
        ...
        if(id>0){
           $('#checkAddBtn').show();
           createSDMarker(myLatLngC, $('#checkpoint' + id + ' option:selected').text(),id);
        }
        ...
        var currMar = srcDestMarkers[k];
	        if (ID == currMar.options.id){
	        	map.removeLayer(currMar);
	        }
        }    
		...
		if (ID == 111){
		img = L.icon({
                iconUrl: greenMarker // Source.
            });
		}else if (ID == 999){
		img =  L.icon({
                iconUrl: orangeMarker // Checkpoint.
            });
		}else{
		img =  L.icon({
                iconUrl: blueMarker // Checkpoint.
            });
		}
