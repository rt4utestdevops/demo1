...
<div class="tabs-container">
   <ul class="nav nav-tabs">
      ...
      <li><a href="#listViewId" id="listView" data-toggle="tab" style="font-size: 15px;font-weight: 600;height:32px;padding-top:4px;">List View</a></li>
      <li class="dropdown">
...
<script>
//history.pushState({ foo: 'fake' }, 'Fake Url', 'SLADashboard#');
var map;
var mcOptions = {
    gridSize: 20,
    maxZoom: 100
        ...
        success: function (result) {
            ...
            for (var i = 0; i < results["MapViewVehicles"].length; i++) {
                if (!results["MapViewVehicles"][i].lat == 0 && !results["MapViewVehicles"][i].lon == 0) {
                    ...
                    var mylatLong = new L.LatLng(results["MapViewVehicles"][i].lat, results["MapViewVehicles"][i].lon);
                }
            }
...
function plotSingleVehicle(vehicleNo, latitude, longtitude, location, gmt, tripStatus, custName, shipmentId, delay1, weather, driverName, etaDest, etaNextPt, routeId, ATD,
    ...
    var content = '<div id="myInfoDiv" seamless="seamless" scrolling="no" style="overflow:auto; height: 100%; color: #000; background:#ffffff; line-height:100%; font-size:11px; font-family: sans-serif;">' +
        '<table>' +
        ...
        '</table>' +
        '</div>';
    contentOne = '<div id="myInfoDiv" seamless="seamless" scrolling="no" style="overflow:auto; height: 100%; color: #000; background:#ffffff;font-size:11px; font-family: sans-serif;">' +
        '<table>' +
    ...
    if (location != 'No GPS Device Connected') {
        bounds.extend(new L.LatLng(latitude, longtitude));
    }
