...
//Shikha was asking to change the icons of the vehicles so please have a word with her on the icons.
                    ...
                    if (!results["MapViewIndex"][i].lat == 0 && !results["MapViewIndex"][i].lon == 0) {
                        ...
                        if(plot) {
                          plotSingleVehicle(results["MapViewIndex"][i].vehicleNo, results["MapViewIndex"][i].lat, results["MapViewIndex"][i].lon,
                            ...
                            results["MapViewIndex"][i].temperature);
    ...
    function plotSingleVehicle(vehicleNo, latitude, longtitude, location, driverName, temperature) {
        var tempContent='';
        var humidityContent='';
        var Humidity;
        ...
        // alert(shipmentId);
        var content = '<div id="myInfoDiv" class="blueGreyLight" seamless="seamless" scrolling="no" style="border: 1px solid #37474F;overflow:hidden; width:100%; height: 140px; float: left; color: #000; line-height:100%; font-size:11px; font-family: sans-serif;padding:4px;">' +
            '<table class="infoDiv">' +
        ...
        google.maps.event.addListener(marker, 'click', (function(marker, contents, infowindow) {
            return function() {
                firstLoadDetails = 1;
                info = false;
                var imageurl = '/ApplicationImages/VehicleImages/delivery-van-green-selected.png';
                if(tripStat == "available")
                {
                  imageurl = '/ApplicationImages/VehicleImages/delivery-van-blue-selected.png';
                }
                if(tripStat == "enroute")
                {
                  imageurl = '/ApplicationImages/VehicleImages/delivery-van-lightblue-selected.png';
                }
                if(tripStat == "waiting")
                {
                  imageurl = '/ApplicationImages/VehicleImages/delivery-van-orange-selected.png';
                }
                if(tripStat == "notReady")
                {
                  imageurl = '/ApplicationImages/VehicleImages/delivery-van-red-selected.png';
                }
                if(tripStat == "onTrip")
                {
                  imageurl = '/ApplicationImages/VehicleImages/delivery-van-green-selected.png';
                }
                image = {
                    url: imageurl, // This marker is 20 pixels wide by 32 pixels tall.
                    scaledSize: new google.maps.Size(40, 40), // The origin for this image is 0,0.
