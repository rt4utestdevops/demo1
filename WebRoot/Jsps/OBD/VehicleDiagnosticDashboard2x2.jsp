...
body {
   ...
   <body onload="loadData()" class="body-bg">
      ...
      <section class="vehicle-diag">
         <div class="container-custom">
         	<div class="pg-title">
               	<div><h2 style="text-align: -webkit-center; float: left; margin-top: 5px; margin-right: -226px" id="ignitionId"></h2></div>
...
<!--   	  	}-->
		...
		function loadRecords(vehicleNo){
        	...
        	success: function(result) {
			 ...
			 //Alert Details	
   	  		$.ajax({
        	url: '<%=request.getContextPath()%>/OBDAction.do?param=getVehicleDiagnosticDeatails2x2',
        	data: {
        		vehicleNo: vehicleNo
        	},
        	success: function(result) {
            	...
            	if(pendingList["vehicleDiagnosisDetailsRoot2x2"].length > 0){
					
					
            		for(var i = 0; i < pendingList["vehicleDiagnosisDetailsRoot2x2"].length; i++){
						
						var temp;
						
						 if(pendingList["vehicleDiagnosisDetailsRoot2x2"][i].color=='icon-green') 
						 temp =  '<img src="../../Main/resources/images/obd/'+pendingList["vehicleDiagnosisDetailsRoot2x2"][i].id+'.png" alt="user">'  ;
						else if(pendingList["vehicleDiagnosisDetailsRoot2x2"][i].color=='icon-red')
						temp =	' <img src="../../Main/resources/images/obd/'+pendingList["vehicleDiagnosisDetailsRoot2x2"][i].id+'_red.png" alt="user">'
					 ...
					 '<div class="diag-items">' +
                       ...
                       '</div>' +
                       '<div class="diag-txt">' +
                          '<p class="line-height">'+pendingList["vehicleDiagnosisDetailsRoot2x2"][i].paramName+'</p>' +
						'</div>' +
						
						 
						
                         '<div class="diag-status">' +
                          '<p>'+pendingList["vehicleDiagnosisDetailsRoot2x2"][i].value+'</p>' +
                         '</div>' +
                       '</div>'  
