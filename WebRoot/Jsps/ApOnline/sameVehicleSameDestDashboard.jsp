...
var selected;
var grid;
var jspName="SameVehicleSameDestination";
var exportDataType = "int,string,string,string,string,string,string,string";
var myWin;
var displaygrid;

var reader = new Ext.data.JsonReader({
    idProperty: 'sameVehicleSameDestinationid',
    root: 'sameVehcileSameDestinationRoot',
    totalProperty: 'total',
    fields: [{
        name: 'slnoIndex'
    }, {
...
var samevehiclestore = new Ext.data.GroupingStore({
    autoLoad: false,
    proxy: new Ext.data.HttpProxy({
        url: '<%=request.getContextPath()%>/SameVehicleSameDestination.do?param=getSameVehicleSameDestinationDetails',
        method: 'POST'
    }),
    storeId: 'storeId',
    reader: reader
...
function closeImportWin(){
	myWin.hide();
	samevehiclestore.removeAll();
}
displaygrid = getGrid('', 'No Records Found', samevehiclestore, screen.width - 168, 440, 10, filters, 'Clear Filter Data', false, '', 20, false, '', false, '', true, 'Excel', jspName, exportDataType, true, 'PDF', false, '', false, '', false, '',false,'',false,'',false,'',false,'',false,'',false,'',false,'',false,'',true,'Close');
...
function onCellClickOnGrid(grid, rowIndex, columnIndex, e) {
  ...
  if (grid.getSelectionModel().getCount() == 1) {
         ...
         var destinationName = selected.get('destinationIndex');
		 var startDate = Ext.getCmp('startdate').getValue();
		 var endDate = Ext.getCmp('enddate').getValue();
		 var vehicleNo = selected.get('vehicleNumberDataIndex');
	samevehiclestore.load({
      	params:{
