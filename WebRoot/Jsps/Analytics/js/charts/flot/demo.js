$(function(){
  ...
  var otp = [];
  for (var i = 0; i <= 6; i += 1) {
    otp.push([i, parseInt((Math.floor(Math.random() * (1 + 30 - 10))) + 10)]);
  }
  var otd = [];
  for (var i = 0; i <= 6; i += 1) {
    otd.push([i, parseInt((Math.floor(Math.random() * (1 + 30 - 10))) + 10)]);
  }
  $("#flot-chart").length && $.plot($("#flot-chart"), [{
      ...
      }],
      {
        ...
        colors: ["#dddddd","#7DCB51","#FFCC00","#D40511"],
        xaxis: {
            ticks: 15,
            tickDecimals: 0
        },
  ...
  $("#flot-bar1").length && $.plot($("#flot-bar1"), dataBar, {
      xaxis: {
