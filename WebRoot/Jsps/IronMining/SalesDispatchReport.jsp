...
if (request.getParameter("list") != null) {
	...
	if (loginInfo == null) {
		...
		int customerId = loginInfo.getCustomerId();
			 ...
			 tobeConverted.add("Select_Month_Year");
			 tobeConverted.add("Mineral_Name");
...
convertedWords = cf.getLanguageSpecificWordForKey(tobeConverted,language);
		    ...
		    String Grade=convertedWords.get(13);
		    String SelectGrade=convertedWords.get(14);
		    String SelectMonthYear=convertedWords.get(15);
		    String SelectMineral=convertedWords.get(16);
		 
%>

<jsp:include page="../Common/header.jsp" />
    <base href="<%=basePath%>">
    <title>SalesDispatchReport</title>
 ...
 <script>
    //-------------------------------------------------------------------------------//
    var outerPanel;
    var jspName = "SalesDispatchReport";
    var json = "";
    var deductionClaimedGrid;
	var exportDataType = "int,string,string,string,string,numeric,numeric,string,numeric,numeric";
	var buttonValue;
	var dtcur = datecur;
    ...
    var gradeStore=new  Ext.data.JsonStore({
	   	 id: 'gradeStoreId',
	   	 url: '<%=request.getContextPath()%>/MonthlyReturnsReportsAction.do?param=getGradeDetails',
	   	 root:'GradeRoot',
   ...
   //-------------------------------------------------------------------------------------------------------//
                           ...
                           var custName=Ext.getCmp('custcomboId').getRawValue();
                            store.load({
                                params: {
									...
									grade: Ext.getCmp('gradeComboId').getRawValue()
    ...
    var reader = new Ext.data.JsonReader({
        idProperty: 'GradeDetailsRootId',
        root: 'GradeDetailsRoot',
        totalProperty: 'total',
        fields: [ {
			            name: 'slnoIndex'
			        }, {
			            ...
			            name: 'DespatchNameIndex',
			            type: 'string'
			        },{
			            name: 'ConsigneeNameIndex',
			            type: 'string'
			        },{
			            name: 'DomesticQuantityIndex',
			            type: 'numeric'
			        },{
			            name: 'SaleValueIndex',
			            type: 'numeric'
    ...
    var store = new Ext.data.GroupingStore({
        ...
        proxy: new Ext.data.HttpProxy({
            url: '<%=request.getContextPath()%>/MonthlyReturnsReportsAction.do?param=getSalesDispatchDetails',
            method: 'POST'
        }),
        remoteSort: false,
        sortInfo: {
    ...
    var filters = new Ext.ux.grid.GridFilters({
        ...
        filters: [  {
            ...
            dataIndex: 'DespatchNameIndex',
            type: 'string'
        }, {
            dataIndex: 'ConsigneeNameIndex',
            type: 'string'
        }, {
            dataIndex: 'DomesticQuantityIndex',
            type: 'numeric'
        }, {
            dataIndex: 'SaleValueIndex',
            type: 'numeric'
        }, {
            dataIndex: 'CountryIndex',
            type: 'string'
        }, {
            dataIndex: 'ExportQuantityIndex',
            type: 'numeric'
        }, {
            dataIndex: 'FOBValueIndex',
            type: 'numeric'
        }
    ...
    var createColModel = function(finish, start) {
        var columns = [
             new Ext.grid.RowNumberer({
                ...
                header: '<b>Consignee Name & Registration Number</b>',
                sortable: true,
                width: 120,
                dataIndex: 'ConsigneeNameIndex',
                filter: {
	...
	Grid = getGrid('Sales Dispatch Details', '<%=NoRecordsfound%>', store, screen.width - 38, 400, 11, filters, '<%=ClearFilterData%>', false, '', 11, false, '', false, '', true, '<%=Excel%>', jspName, exportDataType, false, 'PDF',false,'Add',false,'Modify',false,'',false,'',false,'Generate PDF');
