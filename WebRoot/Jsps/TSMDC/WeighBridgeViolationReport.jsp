...
<script>
	...
	var outerPanel;
	var AssetNo;
 	var jspName='Weigh_Bridge_Violation_Report';
   	var exportDataType = "int,string,string,string,string,number,number,number,number,number,number,date,string";
    var dtprev = dateprev;
	var dtcur = datecur;
	var dtnxt = datenext;
	var reportTypeId;
...
//************************ Combo for Customer Starts Here***************************************//
	...
	var reportTypeStore =  new Ext.data.SimpleStore({
	  							 ...
	  							 data: [['1', 'Weigh Bridge Report'],['2', 'Duplicate Transit Pass']]
							}); 
	...
	var reader = new Ext.data.JsonReader({
        ...
        fields: [{
            ...
            name: 'tareWeightKgDataIndex'
        },{
            name: 'grossWeightKgDataIndex'
        },{
            name: 'netWeightKgDataIndex'
        },{
            name: 'insertDateDataIndex',
        },{
    ...
    var SummaryReportStore = new Ext.data.GroupingStore({
        ...
        proxy: new Ext.data.HttpProxy({
            url: '<%=request.getContextPath()%>/SandTSMDCAction.do?param=getWeighBridgeViolationDetails',
            method: 'POST'
        }),
        remoteSort: false,
    ...
    var filters = new Ext.ux.grid.GridFilters({
        ...
        filters: [{
            ...
            type: 'string',
            dataIndex: 'transitPassQtyDataIndex'
        }, {
            type: 'numeric',
            ...
            type: 'numeric',
            dataIndex: 'tareWeightKgDataIndex'
        }, {
            type: 'numeric',
            dataIndex: 'grossWeightKgDataIndex'
        }, {
            type: 'numeric',
            dataIndex: 'netWeightKgDataIndex'
        }, {
            type: 'date',
            dataIndex: 'insertDateDataIndex'
        },   {
            type: 'string',
    ...
    var createColModel = function (finish, start) {
        ...
        var columns = [
            new Ext.grid.RowNumberer({
                ...
                header: "<span style=font-weight:bold;>Transit Pass Quantity (CuM)</span>",
                dataIndex: 'transitPassQtyDataIndex',
                filter: {
                    type: 'string'
                }
            }, {
                header: "<span style=font-weight:bold;>Tare Weight (CuM)</span>",
                dataIndex: 'tareWeightDataIndex',
                filter: {
                    type: 'numeric'
                }
            }, {
                header: "<span style=font-weight:bold;>Gross Weight (CuM)</span>",
                dataIndex: 'grossWeightDataIndex',
                filter: {
                    type: 'numeric'
                }
            }, {
                header: "<span style=font-weight:bold;>Net Weight (CuM)</span>",
                dataIndex: 'netWeightDataIndex',
                filter: {
                    type: 'numeric'
                }
                ...
                header: "<span style=font-weight:bold;>Weight Difference </span>",
                dataIndex: 'weightDiffDataIndex',
                filter: {
                    type: 'string'
                }
    ...
    summaryGrid = getGrid('', '<%=NoRecordsFound%>', SummaryReportStore, screen.width - 35, 400, 22, filters, '<%=ClearFilterData%>', false, '', 10, false, '', false, '', true, '<%=Excel%>', jspName, exportDataType, true, '<%=PDF%>');
      ...
      Ext.onReady(function () {
        ...
        outerPanel = new Ext.Panel({
        	title:'Weigh Bridge Violation Report',
            renderTo: 'content',
            standardSubmit: true,
            frame: true,
            cls: 'outerpanel',
