...
ArrayList<String> tobeConverted = new ArrayList<String>();
   tobeConverted.add("Vehicle_Utilization_During_Holidays_And_Weekends");
  ...
  tobeConverted.add("Utilized_During_Working_Days");
  tobeConverted.add("Non_Utilization_During_Working_Days");
  tobeConverted.add("Utilized_On_Holidays");
  tobeConverted.add("HRS_of_Ignition_ON");
...
String VehicleUtilizationDuringHolidaysAndWeekends = convertedWords.get(0);
...
String NonUtilizationDuringWorkingDays = convertedWords.get(22);
String UtilizedOnHolidays = convertedWords.get(23);
String HRSofIgnitionON = convertedWords.get(24);
String TravelTime = convertedWords.get(25);
String DistanceTravelled = convertedWords.get(26);
String Utilization = convertedWords.get(27);
String ClearFilterData = convertedWords.get(28);
String Excel = convertedWords.get(29);
String UtilizationDetails = convertedWords.get(30);
String UtilizationGraph = convertedWords.get(31);
String RegistrationNumber= convertedWords.get(32);
String NoRecordsFound= convertedWords.get(33);
String AssetType = convertedWords.get(34);
String Submit = convertedWords.get(35);
String LowestUtilization = convertedWords.get(36);
String HighestUtilization = convertedWords.get(37);
String AssetModel = convertedWords.get(38);
...
<html class="largehtml">
 <head>
 		<title><%=VehicleUtilizationDuringHolidaysAndWeekends %></title>		
</head>	    
	 <style>
  .x-panel-tl
	{
 ...
 google.load("visualization", "1", {packages:["corechart"]});
   ...
   var jspName = "VehicleUtilizationDuringHolidaysAndWeekends";
   var exportDataType = "int,string,string,string,string,int,int,int,int,int,int,float,float,float,int";
 ...
 var assetGroupCombo = new Ext.form.ComboBox({
  ...
  var clientPanel = new Ext.Panel({
   ...
   var reader = new Ext.data.JsonReader({
            ...
            root: 'utilizationDuringHolidayAndWeekendsRoot',
            totalProperty: 'total',
            fields: [{
                name: 'slnoIndex'
            }, {
                ...
                name: 'nonUtilizationDuringWorkingdaysDataIndex'
            }, {
                name: 'UtilizedOnHolidaysDataIndex'
            },{
                name: 'HRSofIgnitionONDataIndex'
            }, {
    ...
    var store = new Ext.data.GroupingStore({
            ...
            proxy: new Ext.data.HttpProxy({
                url: '<%=request.getContextPath()%>/UtilizationReportsAction.do?param=getUtilizationWorkingHolidaysAndWeekendReport',
                 method: 'POST'
            }),
            
            remoteSort: false,
            sortInfo: {field: 'utilizationPersentDataIndex',
            		   direction: 'desc'
            },
            storeId: 'utilizationWorkingHolidaysAndWeekendReportId',
            reader: reader
        });
        ...
        var filters = new Ext.ux.grid.GridFilters({
            ...
            filters: [{
                ...
                type: 'int',
                dataIndex: 'nonUtilizationDuringWorkingdaysDataIndex'
            } ,{
                type: 'int',
                dataIndex: 'UtilizedOnHolidaysDataIndex'
            },{
                type: 'float',
                dataIndex: 'HRSofIgnitionONDataIndex'
            }, {
                type: 'float',
     ...
     var createColModel = function (finish, start) {
            ...
            var columns = [
                new Ext.grid.RowNumberer({
                    ...
                    dataIndex: 'nonUtilizationDuringWorkingdaysDataIndex',
                    header: "<span style=font-weight:bold;><%=NonUtilizationDuringWorkingDays%></span>",
                    width:100,
                    filter: {
                        type: 'int'
                    }
                    ...
                    dataIndex: 'HRSofIgnitionONDataIndex',
                    header: "<span style=font-weight:bold;><%=HRSofIgnitionON%></span>",
                    hidden:true,
                    width:100,
                    filter: {
                        type: 'float'
     ...
     var grid = getGrid('<%=VehicleUtilizationDuringHolidaysAndWeekends%>', '<%=NoRecordsFound%>', store, screen.width - 55, 362, 16, filters, '<%=ClearFilterData%>', false, '', 14, false, '', false, '', true, '<%=Excel%>', jspName, exportDataType, true, 'PDF');
  ...
  //-------------------------function for High utilization Bar Chart---------------------------------------  
     function higestUtilizationbarChart() {
  		var barchartrevenuegraph = new google.visualization.ColumnChart(document.getElementById('utilizationchartdiv'));
  			...
  			for (var i = 0; i < firstTwenty; i++){
				...
				revenuesubdivision=rec.data['assetNumberDataIndex'];
					if(store.getCount()!=1 && i!=store.getCount()-1 && revenuesubdivision==store.getAt(i+1).data['assetNumberDataIndex'])
						{
							revenue=parseInt(revenue)+parseInt(rec.data['utilizationPersentDataIndex']);
							continue;
						}
            ...
            count=0;
			var options = {
                title: '<%=VehicleUtilizationDuringHolidaysAndWeekends%>',
                titleTextStyle: {
                    color: '#686262',
                    fontSize: 12
                },
 ...
 //-------------------------function for low utilization Bar Chart--------------------------------------- 
