			...
			$.ajax({
			  ...
			  success: function(result) {
						...
						tripConfiguration = result.tripDetails[0];
						  $('#statusDropDownId').append($("<option></option>").attr("value", "ALL").text("All"));
						  $('#statusDropDownId').append($("<option></option>").attr("value", "AVAILABLE").text("Available"));
						  $('#statusDropDownId').append($("<option></option>").attr("value", "ON_TRIP").text("On Trip"));
						  $('#statusDropDownId').append($("<option></option>").attr("value", "TRIP_ASSIGNED").text("Trip Assigned"));
						  if(tripConfiguration.enableBorderAlert == "Y"){
							$('#statusDropDownId').append($("<option></option>").attr("value", "AT_BORDER").text("At Border"));
							$('#statusDropDownId').append($("<option></option>").attr("value", "STOPPED").text("Stopped"));
						  }
						}
				})
...
$('#statusDropDownId').on('change', function (e) {
  ...
  initialize("map",new L.LatLng(<%=latitudeLongitude%>),'<%=mapName%>', '<%=appKey%>', '<%=appCode%>');
   setTimeout(function(){map.invalidateSize();map.setView(new  L.LatLng(<%=latitudeLongitude%>),8);layerGroup = new L.LayerGroup().addTo(map)},100);
 ...
 function loadMap(tripStatusIn) {
                    ...
                    if (!results["MapViewIndex"][i].lat == 0 && !results["MapViewIndex"][i].lon == 0) {
                        count++;
                          plotSingleVehicle(results["MapViewIndex"][i].vehicleNo, results["MapViewIndex"][i].lat, results["MapViewIndex"][i].lon,
                            results["MapViewIndex"][i].location, results["MapViewIndex"][i].driverName,results["MapViewIndex"][i].driverContact,
    ...
    function plotSingleVehicle(vehicleNo, latitude, longtitude, location, driverName,driverContact,
		...
		if(iconColour == "Red"){
			...
			imageurl = '/ApplicationImages/VehicleImages/delivery-van-yellow.png';
		}else{
       ...
       // markerClusterArray.push(marker);
        ...
        var content = '<div id="myInfoDiv" class="blueGreyLight" seamless="seamless" scrolling="no" style="border: 1px solid #37474F;overflow:hidden; width:100%; height: 100%; float: left; color: #000; line-height:100%; font-size:11px; font-family: "Tahoma", Geneva,  sans-serif !important;padding:4px;">' +
			...
			'<tr><td nowrap><b>Driver Contact:&nbsp;&nbsp;</b></td><td>' + driverContact + '</td></tr>' +
			...
			var marker = new L.Marker(new L.LatLng(latitude, longtitude), {
				icon: image
                }).bindTooltip(driverName,{permanent: true,direction: 'top',className: 'labels'}).bindPopup(content);
