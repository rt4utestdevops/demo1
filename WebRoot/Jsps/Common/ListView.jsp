...
.multiselect .caret{
	...
	.multiselect { display:flex !important; justify-content: space-between !important; padding:4px;
	}
...
.multiselect-filter .input-group {width:93% !important}
.multiselect-search{border-radius: 8px !important}
...
<!-- <body oncontextmenu="return false;"> -->
	 <jsp:include page="../Common/LiveVisionExportJs.jsp"/>
		...
		<div class="container" style=" margin-top: 1%;">
			...
			<header><h1 style="font-size:2rem !important;color: #007bff !important;font-weight:100 !important;"><%=LiveVision%></h1></header>
            
				<select class="combobox"  style="height:32px !important;background:white;border:1px solid #777777 !important;font-size:14px !important;display:none;" id="cust_names" multiple>
				</select>
				<input class="btn-primary" id="btnView" style="padding:0px 16px;margin:8px 8px;height:28px;border-radius:4px;display:none;" type="button" value="View" onclick="GRID()">			
			</div>
			  <div>
			    
				
				<select class="combobox" id="vehicletype" style="width:140px !important;margin-left:16px;margin-top:8px;height:28px;margin-right:16px;" onchange="GRID();">
...
function getCustNames(cust) {
    ...
    $.ajax({
        ...
        success: function (response) {
			...
			if(custList["customerRoot"].length > 0)
			{
				$("#cust_names").show();
				$("#btnView").show();
				...
				custNameTotalLength = $("#cust_names").val().length;
				GRID();
			}
		}
    });
...
var items = "";
if($("#cust_names").val() == null || $("#cust_names").val().length === custNameTotalLength){
   items = ",ALL";
}
else{
$('#cust_names option:selected').each(function(){ items += "," + $(this).val(); });}
console.log(items);
vehicleDetailsStore.load({
params:{custName:'<%=customernamelogged%>', jspName:jspName,vehicleType:document.getElementById("vehicletype").value,processID:processID,checkFDAS:'<%=checkFDAS%>', custNameList: items} });
var vehtype=document.getElementById("vehicletype").value;
$('#listviewcontainer').attr('src',"<%=path%>/Jsps/Common/MapView.jsp?custNameList="+items+"&vehicleType="+vehtype+"&processId=<%=processID%>");
}
...
function viewFuelPercentage(grid1,rowIndex,columnIndex,e)
 ...
 Ext.onReady(function () {
    ...
    custNameList ="";
     if(previousCustNameList!=null && previousCustNameList!=""){
    ...
    //
	 ...
	 <% }else { %>
	    vehicleDetailsStore.load({
		params:{custName:'<%=customernamelogged%>',jspName:jspName,custNameList:custNameList,vehicleType:vehicleType,processID:processID,checkFDAS:'<%=checkFDAS%>' } });
	<%}%>
	if('<%=language%>'=='en'){
