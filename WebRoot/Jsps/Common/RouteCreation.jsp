...
<head>
  ...
  crossorigin=""></script>
  <link rel="stylesheet" href="../../Main/leaflet/leaflet-draw/css/leaflet_numbered_markers.css"/>
      <script src="../../Main/leaflet/leaflet-draw/js/leaflet_numbered_markers.js"></script>
...
<body onload="ShowRoute()">
 ...
 function plotRoute(latlongorigin, latlongDestination) {
		...
		var source1 = Ext.getCmp('sourcecomboId').getValue();
		var s = SourcecomboStore.find('Hub_Id', source1);
       var s1 = SourcecomboStore.getAt(s);
		var route = new Array();
		var waypoints = 'waypoint0='+latlongorigin.lat +','+latlongorigin.lng;
		createSDMarker(latlongorigin,s1.data['Hub_Name'],'S');
		route.push({lat:latlongorigin.lat ,lng:latlongorigin.lng});
		var counter = 0;
		  for (var j = 0; j < wayPts.length; j++) {
			  ...
			  hubName=arr[3];
              counter = j+1;
			route.push({lat:parseFloat(arr[0].trim()) ,lng:parseFloat(arr[1].trim())});
			 point = new L.LatLng(parseFloat(arr[0].trim()),parseFloat(arr[1].trim()));
			waypoints = waypoints + '&waypoint'+(counter)+'='+parseFloat(arr[0].trim()) +','+parseFloat(arr[1].trim()); 
			createSDMarker(point,hubName,num);
            
        }
		var dest1 = Ext.getCmp('destinationcomboId').getValue();
		var d = destcomboStore.findExact('Hub_Id',dest1);
       var d1 = destcomboStore.getAt(d);
		route.push({lat:latlongDestination.lat ,lng:latlongDestination.lng});
		waypoints = waypoints + '&waypoint'+(counter+1)+'='+latlongDestination.lat +','+latlongDestination.lng;
		createSDMarker(latlongDestination,d1.data['Hub_Name'],'D');
		jsonArray = [];
	 ...
	 routingControl = L.Routing.control({
			  ...
			  routingControl.on('routesfound', function(e) {
			   ...
			   for(var i=0;i < routes[0].instructions.length; i++) {
					checkPointD += routes[0].instructions[i].time + ",";
				}
			   var summary = routes[0].summary;
			   var totalDuration = e.routes[0].summary.totalTime;
			var hours = parseInt(totalDuration /3600);
           var  mod = parseInt(totalDuration % 3600);
			var durationHHMM = hours +"."+mod;
			   Ext.getCmp('actualKmId').setValue((summary.totalDistance / 1000).toFixed(2));
			    Ext.getCmp('actualTimeId').setValue(durationHHMM);
				for (var i = 0;i < routes[0].coordinates.length; i++) {
               jsonArray.push('{' + (i + 1) + ',' + routes[0].coordinates[i].lat + ',' + routes[0].coordinates[i].lng + '}');
			   i = i + 50;
           }
			});
 ...
 var contentString;
	function createInfoWindow(latitudeInfo,longitudeInfo,location,hubId){
			...
			$.ajax({
             ...
             success: function(response) {
                 ...
                 for (i = 0; i <= obj1["checkPoint"].length - 1; i++) {
                 	var myLocLatLng = new L.LatLng(obj1["checkPoint"][i].latitude,obj1["checkPoint"][i].longitude);
       ...
       function clearCheckPoints() {
           ...
           if (directionsDisplay != null) {
			   map.removeLayer(directionsDisplay);
           }
           ...
           for(var i = 0; i < locArray.length; i++){
			 map.removeLayer(locArray[i]);
           }
	       for(var i=0;i<MarkerinfowindowArr.length;i++){
			map.removeLayer(MarkerinfowindowArr[i]);
		   }
		   for (var i = 0; i < circleArray.length; i++) {
			  map.removeLayer(circleArray[i]);
           }
           for (var i = 0; i < polygonCoords.length; i++) {
			   map.removeLayer(polygonCoords[i]);
           }
       ...
       function saveInfoWindowDetails(CPLat, CPLng,ChubId) {
           ...
           var i = 1;
           checkPointArray.push('{' + CheckPointName + ',' + new L.LatLng(CPLat, CPLng) + ',' + CheckRadius + ',' +  CheckType + ',' +  detentionTime + ',' +ChubId +'}');
           wayPts.push(CPLat, CPLng);
       ...
       function plotDrag(result, map, directionsDisplay) {
		  ...
		  if((objLatLngSource.lat()==objLatLngDest.lat()) && (objLatLngSource.lng()==objLatLngDest.lng())){
			...
			Ext.Ajax.request({
				...
				params: {
					...
					sourceName: "("+objLatLngSource.lat+','+objLatLngSource.lng+")",
					destAlias: Ext.getCmp('destinationAliasId').getValue(),
					destName: "("+objLatLngDest.lat+','+objLatLngDest.lng+")",
		...
		function setMarkers(map, viewMarkerLat, viewMarkerLng, type) {
			var markerLatLng = new L.LatLng(viewMarkerLat, viewMarkerLng);
			...
			routedetailsstore.load({
				...
				callback: function() {
					...
					for (i = 0; i < routedetailsstore.getCount(); i++) {
						...
						var alias;
						if (rec.data['type'] == "") {
							var latlon = new L.LatLng(rec.data['lat'], rec.data['long']);
        ...
        listeners: {
            select: {
                fn: function() {
                    ...
                    myLatLng = new L.LatLng(recordO.data['latitude'], recordO.data['longitude']);
					...
					// createSDMarker(objLatLngSource, recordO.data['Hub_Name'],'S');
                    document.getElementById("messageBoxId").innerHTML = "Click on \"Plot Route\" after source and destination are selected";
        ...
        listeners: {
            select: {
                fn: function() {
                    ...
                    myLatLng = new L.LatLng(recordD.data['latitude'], recordD.data['longitude']);
                    ...
                    objLatLngDest=myLatLng;
					 //createSDMarker(objLatLngDest,recordD.data['Hub_Name'],'D');
   ...
   var polygonCoords1=[];
    polygonStoreForModify.load({
       params: {
           ...
           callback: function() {
               ...
               polygonNew = new L.Polygon(polygonCoords1,{
                   strokeColor: '#A7A005',
                   strokeOpacity: 0.8,
                   strokeWeight: 3,
                   fillColor: '#ECF086',
	...
	var searchPanel = new Ext.Panel({
 	...
 	function drawCheckpoints() {
          ...
          myLatLngD = new L.LatLng($('#deliveryId option:selected').attr("latitude"), $('#deliveryId option:selected').attr("longitude"));
          ...
          loc1=loc[0];
		  name=$('#deliveryId option:selected').text().split(',').join('_');
          ...
          var i = 1;
          checkPointArray.push('{' + CheckPointName + ',' + myLatLngD.lat +','+myLatLngD.lng + ',' + CheckRadius + ',' +  CheckType + ',' +  detentionTime + ',' +document.getElementById('deliveryId').value +'}');
          wayPts.push($('#deliveryId option:selected').attr("latitude")+","+$('#deliveryId option:selected').attr("longitude")+","+count+","+name);
           var idO = SourcecomboStore.find('Hub_Id',  Ext.getCmp('sourcecomboId').getValue());
           var recordO = SourcecomboStore.getAt(idO);
           myLatLng = new L.LatLng(recordO.data['latitude'], recordO.data['longitude']);
           ...
           myLatLng1 = new L.LatLng(recordd.data['latitude'], recordd.data['longitude']);
	            ...
	            listeners: {
	                click: {
	                    fn: function() {
	                        ...
	                        myLatLng = new L.LatLng(sourceInputarr[0],sourceInputarr[1]);
	                        objLatLngSource=myLatLng;
	            ...
	            listeners: {
	                click: {
	                    fn: function() {
	                        ...
	                        myLatLng = new L.LatLng(destInputarr[0],destInputarr[1]);
	                        objLatLngDest=myLatLng;
...
var bottomPanel = new Ext.Panel({
 ...
 var mainPanel = new Ext.Panel({
	...
	function plotMarkerBasedLatlong(){
        ...
        myLatLng = new L.LatLng(inputarr[0],inputarr[1]);
 ...
 function createSDMarker(latlng, title,ID) {
        ...
        // map.removeLayer(srcDestMarkers[k]);
        var currMar = srcDestMarkers[k];
	        if (ID == currMar._leaflet_id){
	        	 layerGroup.removeLayer(currMar);
	        }
        }    
		
			var marker = new L.Marker(latlng, {
      		  icon:	new L.NumberedDivIcon({number: ID})
	        });
