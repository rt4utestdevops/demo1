...
<html>
 <head>
  ...
  <body>
   ...
   <script>
var outerPanel;
var exportDataType="int,int,string,string,string,string,float,float,string,string,string,string,int";
var jspName = "Route_Master_Details_Report";
var grid;
var buttonValue;
...
var clientcombostore = new Ext.data.JsonStore({
    url: '<%=request.getContextPath()%>/routeCreationAction.do?param=getCustomer',
    id: 'CustomerStoreId',
    root: 'CustomerRoot',
    autoLoad: true,
    remoteSort: true,
...
function addRecord() {
    ...
    buttonValue = '<%=Add%>';
    parent.Ext.getCmp('routeCreationId').enable();
	parent.Ext.getCmp('routeCreationId').show();
	parent.Ext.getCmp('routeOperationId').disable();
	var value='/Telematics4uApp/Jsps/Common/RouteCreation.jsp?custId='+custId+'&buttonValue='+buttonValue+'&custName='+custName;
	parent.Ext.getCmp('routeCreationId').update("<iframe style='width:100%;height:530px;border:0;' src='"+value+"'></iframe>");
}
function modifyData() {
      var custId=Ext.getCmp('custcomboId').getValue();
   ...
   //   var trigger2=selected.get('trigger2DataIndex');
	...
	parent.Ext.getCmp('routeOperationId').disable();
	var custId = Ext.getCmp('custcomboId').getValue();
	var value1='/Telematics4uApp/Jsps/Common/RouteCreation.jsp?custId='+custId+'&buttonValue='+buttonValue+'&routeId='+routeId+'&routeName='+routeName+'&source='+source+'&destination='+destination+'&actualDistance='+actualDistance+'&expDistance='+expDistance+'&actualduration='+actualduration+'&expDuration='+expDuration+'&routeDesp='+routeDesp+'&radius='+radius;
	parent.Ext.getCmp('routeCreationId').update("<iframe style='width:100%;height:530px;border:0;' src='"+value1+"'></iframe>");
  }
...
var store = new Ext.data.GroupingStore({
    ...
    proxy: new Ext.data.HttpProxy({
        url: '<%=request.getContextPath()%>/routeCreationAction.do?param=getRouteDetails',
        method: 'POST'
    }),
    storeId: 'storeId',
    reader: reader
...
<!--        },-->
        {
            ...
            header: "<span style=font-weight:bold;>Expected Distance</span>",
            dataIndex: 'tempDistanceDataIndex',
            width: 100,
            filter: {
                type: 'numeric'
...
//*****************************************************************Grid *******************************************************************************
grid = getGrid('', '<%=NoRecordsFound%>', store, screen.width - 40, 420, 30, filters, '<%=ClearFilterData%>', false, '', 16, false, '', false, '', true, 'Excel', jspName, exportDataType, false, 'PDF', true, '<%=Add%>', true, 'View', false, '',false, 'Inactive');
//******************************************************************************************************************************************************
