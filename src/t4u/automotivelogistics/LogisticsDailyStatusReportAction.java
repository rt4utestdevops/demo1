...
public class LogisticsDailyStatusReportAction extends HttpServlet {
	int systemId=0;
	String clientId="";
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
		    ...
		    reportdataList=alf.getActionLogDetails(clientId,systemId,startDate,endDate,offset);
		    ArrayList<String> startTitleList = new ArrayList<String>();
			startTitleList.add("Action Log Report For - "+clientName);
			int noOfLinePerSheet = reportdataList.size();
			ArrayList footers = new ArrayList();
			footers.add("service provided by "+" - "+"T4U");
			ArrayList<String> headerList = getHeaderList(language);
	...
	private void printBill(HttpServletResponse response,
		...
		try
		{
			...
			String dailyStatusReport=p.getProperty("historicalTripReport");
			FileInputStream fis = new FileInputStream(dailyStatusReport+"ActionLogReport_"+systemId+"_"+clientId+".xls");
	...
	@SuppressWarnings("unchecked")
private ArrayList<String> getHeaderList(String lang) {
		...
		try {
			...
			for(int i=1;i<=25;i++){
				...
				headerList.add(cf.getLabelFromDB("Planned_Time",lang));
				headerList.add(cf.getLabelFromDB("Actual_Time",lang));
