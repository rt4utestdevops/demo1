 ...
 */
public class ExportExcelDriverBehaviourReport 
{
	...
	Colour GREEN_BKG = Colour.LIGHT_GREEN;
	Colour YELLOW_BKG = Colour.YELLOW;
	Colour RED_BKG = Colour.CORAL;
	...
	public void createDataRow(ArrayList rowList,WritableSheet sheet)
	{
		try
		{
		    ...
		    int col = cellStart;
		    	twoDigitsAfterPt = new WritableCellFormat(dataWF,dp3.FORMAT3);
			    double score = Double.parseDouble(rowList.get(size-4).toString());
			    if(score<2){
			    	cf.setBackground(GREEN_BKG);
			    	firstCell.setBackground(GREEN_BKG);
			    	twoDigitsAfterPt.setBackground(GREEN_BKG);
				}
				 else if(score>=2 && score<5){
					 cf.setBackground(YELLOW_BKG);
					 firstCell.setBackground(YELLOW_BKG);
					 twoDigitsAfterPt.setBackground(YELLOW_BKG);
		    	 }
		    	 else if(score>=5 ){
		    		 cf.setBackground(RED_BKG);
		    		 firstCell.setBackground(RED_BKG);
		    		 twoDigitsAfterPt.setBackground(RED_BKG);
		    	 }
		    for(int i = 0; i < size; i ++)
		    {
