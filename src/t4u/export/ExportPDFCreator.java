 ...
 *
 */
public class ExportPDFCreator extends PdfPageEventHelper {
	...
	String language;
	PdfPTable pt;
	public ExportPDFCreator() {
	}

	public ExportPDFCreator(ArrayList<String> startTitleList,
...
*/			String summaryList="Summary List";
	       ...
	       // System.out.println("dataTypeList == "+dataTypeList);
			//System.out.println("dataHeaderList == "+dataHeaderList);
			//System.out.println("dataTypeListsize == "+dataTypeList.size());
			//System.out.println("dataHeaderListsize == "+dataHeaderList.size());
		    
			if(dataTypeList.size() == dataHeaderList.size()){
			
			for (int j = 0; j < dataList.size(); j++) {
				int n=Integer.parseInt(((ArrayList)dataList.get(j)).get(0).toString());
				
				if(n%2==0){
					 bs=new BaseColor(219,229,241);
				}else{
				...
				for (int i = 0; i < dataheaderlistsize; i++) {
					...
					if (language.equals("ar")) {
						...
						}else if(dataTypeList.get(i).equalsIgnoreCase("int")){
						     cellAr.setHorizontalAlignment(Element.ALIGN_CENTER);
						}else if(dataTypeList.get(i).equalsIgnoreCase("numeric")){
    ...
    public void createPDFForLiveVision() {
	try {
...
*/			String summaryList="Summary List";
		...
		// need to set the width
		float[] widths = getLocationAdjustedCellWidths(dataHeaderList);
		...
		for (int j = 0; j < dataList.size(); j++) {
			...
			for (int i = 0; i < dataheaderlistsize; i++) {
				...
				if (language.equals("ar")) {
					...
					String datastr = ( (ArrayList) dataList.get(j)).get(i).toString();
					if(datastr.length()>151){
	...
	public float[] getLocationAdjustedCellWidths(ArrayList headerList) {
		...
		float incrWidth = 15.0f;
		float factor = incrWidth / (headerSize - 1);
		boolean locationExists = false;
		for (int i = 0; i < headerSize; i++) {
			if (headerList.get(i).toString().equalsIgnoreCase("location")) {
				locationExists = true;
			}
		}
		...
		if (locationExists) {
			for (int i = 0; i < headerSize; i++) {
				if (headerList.get(i).toString().equalsIgnoreCase("location")) {
					widths[i] = normalWidth + incrWidth;
				} else {
					widths[i] = normalWidth - factor;
				}
			}
		} else {
			for (int i = 0; i < headerSize; i++) {
